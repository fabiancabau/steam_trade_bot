...
app.get('/inventory', function(request, response) {
    ...
    offers.loadInventory(appid.CSGO, contextid.CSGO, true, function (err, inventory){
        if (err) {
            logger.error(err);
            logger.error('You probably dont have this game on your steam.');
        } 
        else {
            ...
            client.hkeys("inventory_item", function (err, items) {
                if (items.length > 0) {
                    console.log('tem item no cache');
                    items.forEach(function (item, i) {
                        inventory_items.push(JSON.parse(item));
                    });
                    ...
                    inventory.forEach(function(item, i) {
                        console.log('Adicionando item no cache');
                        client.hset("inventory_item", JSON.stringify(item), item.id , redis.print);
                    });

                    inventory_items = inventory;
                }   
                
                response.json(inventory_items);
            });
